...
- MobileCoach's paste-time validator scans the raw script text (comments included) for `$` signs and rejects the paste unless each one starts a declared variable name — so no `${…}` template interpolation (use `+` concatenation) and no decorative `$` in comments (enforced by `test/MobileCoachPlatformConstraints.test.js` and the edit-time hook).
