...
//    ones missing from the variable table in docs/mobilecoach-admin-guide.md. A variable that is used
//    by the script but not declared in MobileCoach makes the whole script fail silently (see
//    "Variables" in docs/mobilecoach-admin-guide.md), so a new one must never slip in undocumented.
...
// not in the MobileCoach Admin guide's table (and thus not declared in MobileCoach).
...
function main() {
  ...
  const doc = readFileSync(fileURLToPath(new URL('docs/mobilecoach-admin-guide.md', root)), 'utf8');
  const messages = [];

  const missing = findUndocumentedVariables(src, doc);
  if (missing.length > 0) {
    messages.push(
      `Wrapper variable(s) not documented in the variable table in docs/mobilecoach-admin-guide.md: ${missing.map(n => `$${n}`).join(', ')}.\n` +
      '1. Add them to the table in docs/mobilecoach-admin-guide.md ("One-time MobileCoach setup").\n' +
