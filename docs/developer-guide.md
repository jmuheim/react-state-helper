...
One non-state id is enterable too: `enter('modulesMenu')` (the `MODULE_OVERVIEW_DIALOG_ID` constant, the target of the sessions menu's back entry — see "Menus are static by default" below) resets the whole navigation to `null` — the participant has left the module. It cannot collide with a state id because `registerId` requires an uppercase letter after the level letter.
...
The sessions and activities menus append a **back entry** in the slot after their last item: the sessions menu links `Zurück zur 🗂️ Modulauswahl` to the module-overview dialog (the `MODULE_OVERVIEW_DIALOG_ID` constant, `modulesMenu` — the MobileCoach dialog showing the modules menu must carry exactly that id), the activities menu links `Zurück zu 🗂️ <module title>` to the parent module's dialog. Because a back tap runs through the same generic `enter(…)` follow-up rule as any other tap, `enter('modulesMenu')` is supported and resets the navigation entirely (see "Navigation model" above). To guarantee the back entry always has a free slot, state validation caps sessions per module and activities per session at `MAX_MENU_SLOTS - 1` (8); the module count stays capped at 9, since the top-level menu has no back entry.
