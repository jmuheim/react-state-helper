...
One id the menus emit is *not* enterable: `allModulesMenu` (the target of the sessions menu's back entry — see "Menus are static by default" below). No flow ever calls `enter('allModulesMenu')`: dialogs enter themselves after navigation, and while the modules menu is displayed the participant's tracked location deliberately stays in the previous context — it changes only when the tapped module's dialog runs `enter('<own id>')`. A dedicated guard at the top of `enter()` rejects the id with an explanatory error ("allModulesMenu must never be entered: …") rather than the generic malformed-id message, which would mislead — this is an id the library itself emits into the menu variables. It cannot collide with a state id: `registerId` rejects it by name with its own dedicated error, and the naming convention (uppercase letter after the level letter) would reject it anyway.
