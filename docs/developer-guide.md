...
One non-state id is enterable too: `enter('modulesMenu')` (the `MODULES_MENU_DIALOG_ID` constant, the target of the sessions menu's back entry — see "Menus are static by default" below) resets the whole navigation to `null` — the participant has left the module. It cannot collide with a state id because `registerId` requires an uppercase letter after the level letter.
