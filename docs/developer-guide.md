...
One id the menus emit is *not* enterable: `modulesMenu` (the `MODULES_MENU_DIALOG_ID` constant, the target of the sessions menu's back entry — see "Menus are static by default" below). No flow ever calls `enter('modulesMenu')`: dialogs enter themselves after navigation, and while the modules menu is displayed the participant's tracked location deliberately stays in the previous context — it changes only when the tapped module's dialog runs `enter('<own id>')`. The id cannot collide with a state id because `registerId` requires an uppercase letter after the level letter.
