...
One id the menus emit is *not* enterable: `modulesMenu` (the target of the sessions menu's back entry — see "Menus are static by default" below). No flow ever calls `enter('modulesMenu')`: dialogs enter themselves after navigation, and while the modules menu is displayed the participant's tracked location deliberately stays in the previous context — it changes only when the tapped module's dialog runs `enter('<own id>')`. A dedicated guard at the top of `enter()` rejects the id with an explanatory error ("modulesMenu must never be entered: …") rather than the generic malformed-id message, which would mislead — this is an id the library itself emits into the menu variables. It cannot collide with a state id because `registerId` requires an uppercase letter after the level letter.
...
The sessions and activities menus append a **back entry** in the slot after their last item: the sessions menu links `Ein anderes 🗂️ Modul wählen` to the module-overview dialog (the reserved id `modulesMenu` — the MobileCoach dialog showing the modules menu must carry exactly that id), the activities menu links `Eine andere 📑 Session wählen` to the parent module's dialog. A back tap involves no library command: MobileCoach navigates to the target dialog, and the participant's tracked location stays in the previous context until the next dialog enters itself (see "Navigation model" above). To guarantee the back entry always has a free slot, state validation caps sessions per module and activities per session at `MAX_MENU_SLOTS - 1` (8); the module count stays capped at 9, since the top-level menu has no back entry.
