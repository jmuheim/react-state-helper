...
> Refined by #55: the two audience pages have since been consolidated — the content editor guide is a stub again pending a focused rewrite, and the merged guide (including the variable table the hook and test check) is renamed `docs/maintainer-guide.md`.
...
> Refined by #55: the two guides have since been consolidated into one, renamed `docs/maintainer-guide.md` (the content editor guide is a stub pending a focused rewrite).
...
## 55. The merged guide is the maintainer guide — the audiences are developer, maintainer, and content editor

**Decision:** `docs/developer-guide.md` is renamed `docs/maintainer-guide.md`, and the docs now name three audiences: **developers** change the code logic (internals are documented in the source itself, per the outward-facing rework); **maintainers** sit in between — they change the app content (the state JSON), copy the script into MobileCoach, and create/update the dialog structures and variables there, without touching code logic; **content editors** only fill the created dialogs with their content (`docs/content-editor-guide.md` stays a stub until their focused guide exists). The merged guide's content — MobileCoach setup, variable table, common tasks, cheat-sheet, menus, troubleshooting — is maintainer work, so the guide carries that name. Every reference was updated in the same pass (edit-time hook, constraint tests, README, CLAUDE.md, docs index, constraints-page links, PR template, backlog, open questions); past decision entries keep the old name (append-only); the old GitHub Pages URL gets no redirect stub. Also in the same pass, outward-facing information dropped during this branch's merge-and-condense work was restored into the guide: the back-entry dialog-naming imperatives (`allModulesMenu` / `allSessionsOfCurrentModuleMenu`) with their silent-failure trap and back-tap semantics, the emoji legend, the `$participantGroup` format, the progress advice's adaptation to the deepest entered level, the id-format rationale, and the troubleshooting diagnosis hints.

**Why:** after the merge-and-condense work the guide documents no code internals — everything left is MobileCoach-side work, exactly the maintainer's job, and the "developer" name misled the one audience it claimed to serve. **Rejected:** keeping the "developer guide" name (its recipes are MobileCoach-side tasks and it deliberately documents no internals); splitting a separate developer page off now (nothing substantial would land there — the source and CLAUDE.md already serve developers).

**Watch for:** the "Changing behavior" recipe stays in the maintainer guide although it is strictly developer territory (flagged as such inline) — if real developer documentation ever accumulates, that recipe is the seed of a separate page. The future content editor guide should pull its material (menus, emoji legend, cheat-sheet) from the maintainer guide rather than re-grow a copy that can drift.
