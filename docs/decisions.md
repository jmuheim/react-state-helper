...
## 38. Sessions and activities menus append an automatic back entry; `menuModules` is the reserved module-overview dialog id

**Decision:** `populateMenuWithSessions()` appends `Zurück zur 🗂️ Modulauswahl` in the slot after the last session, routing to the module-overview dialog — a new `MODULE_OVERVIEW_DIALOG_ID` constant, value `menuModules`; the MobileCoach dialog that shows the modules menu must carry exactly that id. `populateMenuWithActivities()` appends `Zurück zu 🗂️ <module title>`, routing to the parent module's own dialog. `enter('menuModules')` is a supported command that resets the whole navigation to `null` (the participant has left the module). To guarantee the back entry a free slot, state validation now caps sessions per module and activities per session at `MAX_MENU_SLOTS - 1` (8); modules stay capped at 9, since the top-level menu has no back entry.

**Why:** without back entries a participant who tapped into a module or session had no way back up the hierarchy — menus are the only navigation surface. Appending inside the populate commands keeps the editor workflow unchanged (same one command per menu). Supporting `enter('menuModules')` is what keeps #28's payoff intact: the same generic `enter(…)` follow-up rule runs after *any* menu tap, back taps included — and resetting the location is the honest semantics of standing in the module overview. `menuModules` cannot collide with a state id because `registerId` requires an uppercase letter after the level letter, so no reserved-word check is needed. **Rejected:** making the back-target dialog id a command parameter — more editor-facing surface for a value that never varies; letting `enter('menuModules')` throw — would force editors to special-case the follow-up rule after every menu that has a back entry, and would leave `$participantGroup` carrying a stale location after backing out; keeping the 9-item caps and appending the back entry only when a slot happens to be free — a menu whose back entry silently disappears at exactly 9 items is a content bug waiting to be discovered live.

**Watch for:** in MobileCoach, the module-overview dialog must be (re)named `menuModules` before the next copy-over, or both back entries' sessions-menu route leads nowhere. Existing state definitions with 9 sessions or 9 activities per parent now fail validation (fine while nothing is deployed). After backing out via `enter('menuModules')`, `$rsh_progressAdvice` is `''` and `$participantGroup` is `null` — same as a fresh participant.
