...
> Refined by #55: the two audience pages have since been consolidated — the author-facing guide is a stub again pending a focused rewrite (renamed `docs/mobilecoach-author-guide.md`), and the merged guide (including the variable table the hook and test check) is renamed `docs/mobilecoach-admin-guide.md`.
...
> Refined by #55: the two guides have since been consolidated into one, renamed `docs/mobilecoach-admin-guide.md` (the author-facing guide, now `docs/mobilecoach-author-guide.md`, is a stub pending a focused rewrite).
...
> Refined by #54: the id-taking query commands the watch-for deferred are removed outright; the open question now concerns the wrapper-plumbing tier.
...
## 54. The id-taking query commands are removed as dead surface

**Decision:** The five query commands taking an explicit id — `isModuleCompleted()`, `isSessionCompleted()`, `hasSessionAdequateProgress()`, `getModuleProgress()`, `hasModuleAdequateProgress()` — are deleted from `ReactStateHelper`, along with their tests and their appearances as `$rsh_cmd` examples (wrapper comment, developer guide). The cascade is removed too: `Module.getProgress()`, `Module.hasAdequateProgress()`, and `Session.hasAdequateProgress()` had no remaining callers — `getProgressAdvice()` compares completed counts against the thresholds directly. Settles what #53's watch-for deferred, without waiting for the batch-commands backlog item.

**Why:** they were pure escape hatches: no internal callers, no MobileCoach flow ever issued one, and their only output channel (`$rsh_result`) is never read by any rule. Dead public surface has real cost here — every public method is `eval`-reachable via `$rsh_cmd` and thus part of the de-facto editor-facing API the docs had to explain away ("should not be necessary at all"), and script size is a suspected MobileCoach performance burden (see the lighten-the-script backlog items). Nothing is deployed (testing phase), so removal is free; if branching on a *non-current* item's completion is ever needed, the code is one `git revert` away — and would then deserve a documented cheat-sheet row with a decided output channel, not an undocumented side door. The threshold/aggregation logic stays covered: `getProgressAdvice` tests exercise both thresholds, `isCurrent…Completed` and `getCompletionOverview` tests cover completion roll-up. **Rejected:** promoting them to documented cheat-sheet commands (no use case today, and `$rsh_result` semantics are unsettled); keeping them undocumented "just in case" (exactly the kinda-private ambiguity #53 identified).

**Watch for:** `$rsh_result` is now `''` on every cheat-sheet command — all remaining doer commands return nothing — which strengthens the batch-commands item's "remove `$rsh_result` entirely" option. #51's watch-for scenario (a raw boolean reaching `$rsh_result`) can now only arise via the wrapper-plumbing getters (`isCurrent…Completed()`), whose `$rsh_cmd`-reachability is the open question this entry narrows the field to. The deployed MobileCoach copy keeps the old methods until the next copy-over — harmless, since nothing calls them there either.

## 55. The merged guide is the MobileCoach Admin guide — the audiences are JavaScript Developer, MobileCoach Admin, and MobileCoach Author

**Decision:** `docs/developer-guide.md` is renamed `docs/mobilecoach-admin-guide.md` (and the old content editor stub `docs/mobilecoach-author-guide.md`), and the docs now name three audiences: **JavaScript Developers** change the code logic (internals are documented in the source itself, per the outward-facing rework); **MobileCoach Admins** sit in between — they change the app content (the state JSON), copy the script into MobileCoach, and create/update the dialog structures and variables there, without touching code logic; **MobileCoach Authors** only fill the created dialogs with their content (`docs/mobilecoach-author-guide.md` stays a stub until their focused guide exists). The merged guide's content — MobileCoach setup, variable table, common tasks, cheat-sheet, menus, troubleshooting — is MobileCoach Admin work, so the guide carries that name. Every reference was updated in the same pass (edit-time hook, constraint tests, README, CLAUDE.md, docs index, constraints-page links, PR template, backlog, open questions); past decision entries keep the old names (append-only); the old GitHub Pages URLs get no redirect stubs. Also in the same pass, outward-facing information dropped during this branch's merge-and-condense work was restored into the guide: the back-entry dialog-naming imperatives (`allModulesMenu` / `allSessionsOfCurrentModuleMenu`) with their silent-failure trap and back-tap semantics, the emoji legend, the `$participantGroup` format, the progress advice's adaptation to the deepest entered level, the id-format rationale, and the troubleshooting diagnosis hints.

**Why:** after the merge-and-condense work the guide documents no code internals — everything left is MobileCoach-side work, exactly the Admin's job, and the "developer" name misled the one audience it claimed to serve. The role names are platform-anchored on purpose: each names the tool it operates ("JavaScript Developer", "MobileCoach Admin", "MobileCoach Author"), and the guide filenames follow the existing `mobilecoach-*` doc naming. **Rejected:** keeping the "developer guide" name (its recipes are MobileCoach-side tasks and it deliberately documents no internals); splitting a separate JavaScript Developer page off now (nothing substantial would land there — the source and CLAUDE.md already serve them); the tool-agnostic role names "maintainer" and "content editor" (an early draft of this entry — replaced before merge because they don't say *which system* the role works in).

**Watch for:** the "Changing behavior" recipe stays in the MobileCoach Admin guide although it is strictly JavaScript Developer territory (flagged as such inline) — if real developer documentation ever accumulates, that recipe is the seed of a separate page. The future MobileCoach Author guide should pull its material (menus, emoji legend, cheat-sheet) from the Admin guide rather than re-grow a copy that can drift.
