...
**Decision:** `populateMenuWithSessions()` appends `Zurück zur 🗂️ Modulauswahl` in the slot after the last session, routing to the module-overview dialog — the reserved id `modulesMenu` (a plain literal at its single use site, not a constant); the MobileCoach dialog that shows the modules menu must carry exactly that id. `populateMenuWithActivities()` appends `Zurück zu 🗂️ <module title>`, routing to the parent module's own dialog. `modulesMenu` is a pure routing target: `enter('modulesMenu')` is never called and not supported (it errors like any unknown id). To guarantee the back entry a free slot, state validation now caps sessions per module and activities per session at `MAX_MENU_SLOTS - 1` (8); modules stay capped at 9, since the top-level menu has no back entry.
