...
const doc = readFileSync(fileURLToPath(new URL('../docs/maintainer-guide.md', import.meta.url)), 'utf8');
...
describe('MobileCoach platform constraints', () => {
  ...
  describe("script-editor validation on save (MobileCoach scans the raw text for $ signs — code, comments, everything — and rejects the save unless each one starts a declared variable; verified 2026-07-09: even '$-prefixed' in a comment was rejected)", () => {
    it('every $ in the source starts a variable name documented in the maintainer guide', () => {
      expect(findInvalidDollarSigns(src, doc)).toEqual([]);
    });
  });

  describe('wrapper variables stay in sync with the variable table in docs/maintainer-guide.md', () => {
    ...
    it('documents every wrapper variable in the maintainer guide (an undeclared variable fails silently in MobileCoach)', () => {
