...
const doc = readFileSync(fileURLToPath(new URL('../docs/mobilecoach-admin-guide.md', import.meta.url)), 'utf8');
...
describe('MobileCoach platform constraints', () => {
  ...
  describe("script-editor validation on save (MobileCoach scans the raw text for $ signs — code, comments, everything — and rejects the save unless each one starts a declared variable; verified 2026-07-09: even '$-prefixed' in a comment was rejected)", () => {
    it('every $ in the source starts a variable name documented in the MobileCoach Admin guide', () => {
      expect(findInvalidDollarSigns(src, doc)).toEqual([]);
    });
  });

  describe('wrapper variables stay in sync with the variable table in docs/mobilecoach-admin-guide.md', () => {
    ...
    it('documents every wrapper variable in the MobileCoach Admin guide (an undeclared variable fails silently in MobileCoach)', () => {
